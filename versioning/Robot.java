...
//		autoKD = SmartDashboard.getNumber("DB/Slider 2", 0);
	...
	public void autonomousInit() {
		...
		straightSum = 0;
		speedSum = 0;
		setShootPower = 0.55;  // 0.75 originally for real bot
		autoShootSpeed = 1400;
		leftOn = false;
		rightOn = false;
		blinker.set(true);
...
//		speedSum = 0;
	...
	public void teleopPeriodic() {
		...
		/*
		else if (driveStick.getRawButton(7)) {   //  shake robot
			loopCount++;
			if (loopCount >= 0 && loopCount < 15) turnLeftSlowly();
			else if (loopCount >= 15 && loopCount < 18) stopRobot();
			else if (loopCount >= 18 && loopCount < 33) turnRightSlowly();
			else {
				stopRobot();
				if (loopCount == 37) loopCount = 0;
			}
//			stayFirm(0);
		}
		
		
		else if (driveStick.getRawButton(11))    {  // test turn to angle
...
//			loopCount++;
		...
		else autoSpeedControl(teleopShootSpeed);   // 1950 for original shooter sprockets;  1200 for boiler spot
...
//			double currentSpeed = shootSpeed.getRate(); 
		...
		else /* if (!driveStick.getRawButton(12)) */ shootBall.set(0);
	}
	...
	void autoOuterGear() {     
		...
		case 2:   //    turn toward peg
			if (redSide) {
				...
				if (rightLight.get() || currentYaw < -53.0) {
					autoStep = 3;
					resetEncoders();
					stopDrive();
					setYaw = currentYaw - 2.0;
					SmartDashboard.putNumber("autoyaw", setYaw);
					autoLoop = 0;
				}
				else driveRobot(-0.35,0.35);
...
//				}
	...
	void autoBoilerGearRed() {  //  red side
		...
		case 2:   //     turn toward the peg
			if (rightLight.get()||currentYaw<-53) {
				autoStep = 3;
				resetEncoders();
				stopDrive();
				setYaw = navX.getYaw() - 2.0;
				SmartDashboard.putNumber("autoyaw", setYaw);
				autoLoop = 0;
			}
...
//				stopRobot();
	...
	//kDist is 1.25 for locoMOEtive, 1 for lawnMOEr
	final double kDist = 1.25;
	public double getDist(){
		double r = distanceR.getRaw();
		double l = distanceL.getRaw();
		return kDist*(Math.abs(l)>Math.abs(r)?l:r);
	}
	public double getDistR(){
		return kDist*(distanceR.getRaw());
	}
	public double getDistL(){
		return kDist*(distanceL.getRaw());
	}
	public double getAvgDist(){
		return kDist*(distanceR.getRaw()+distanceL.getRaw())/2.0;
	}
